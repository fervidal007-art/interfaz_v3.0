...
async def websocket_endpoint(ws: WebSocket):
    ...
    bat = motor.read_battery()
    if bat is not None:
        logger.info(f"🔋 Batería: {bat['mv']} mV ({bat['voltage']} V) {bat['percent']}% — I2C OK")
        await ws.send_json({"type": "battery", "voltage": bat["voltage"], "percent": bat["percent"]})
    else:
